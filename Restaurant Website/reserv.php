...
<body class="header">
    ...
    <nav>
        ...
        <a href="#" class="logo">North Indian Restaurant </a>
        ...
        if (isset($_POST['submit'])) {
            ...
            if (!empty($name) || !empty($number) || !empty($email) || !empty($date) || !empty($time) || !empty($people) || !empty($message)) {
                ...
                if (mysqli_connect_error()) {
                    ...
                    $SELECT = "SELECT email FROM reserv WHERE BINARY email = ? LIMIT 1";
                    $INSERT = "INSERT Into reserv(name,number,email,date,time,people,message) values(?,?,?,?,?,?,?)";
                    ...
                    if ($rnum == 0) {
                        ...
                        $stmt->bind_param("sissiis", $name, $number, $email, $date, $time, $people, $message);
                        $stmt->execute();
